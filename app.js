(async function () {
  ...
  for (const c of CATS) if (events.some(e => e.category === c)) $("cat").insertAdjacentHTML("beforeend", `<option value="${esc(c)}">${esc(LABEL[c] || c)}</option>`);
  ...
  function render() {
    ...
    for (const [k, list] of byCity) {
      ...
      const small = top === "ITF";
      L.circleMarker([c.lat, c.lon], { radius: (small ? 3 : 5) + Math.min(list.length, 6) * (small ? .7 : 1.5), color: "#333", weight: small ? .5 : 1, fillColor: COLOR[top], fillOpacity: .85 }).addTo(layer)
    ...
    $("list").innerHTML = shown.length ? [...byWeek.keys()].sort().map(w => `
      ...
      <table>${byWeek.get(w).sort((a, b) => CATS.indexOf(a.category) - CATS.indexOf(b.category) || a.name.localeCompare(b.name)).map(e => `
        <tr><td class="when">${fmt(e.start)}–${fmt(e.end)}</td>
            <td>${esc(e.name)} <span class="tag">${esc(tag(e))}${e.prize ? ", " + esc(e.prize) : ""}${e.venue ? ", " + esc(e.venue) : ""}</span></td>
